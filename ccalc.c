...
main(int argc, char **argv, char *envp)
{
  ...
  for (cmd = &CALC_DATA_BEGIN;  // initialize the cmd variable with the address
    ...
    VPRINT("result: 0x%llx (%lld) SUM_POSITIVE=%lld SUM_NEGATIVE=%lld\n"
	   ...
	   i,
           cmd, cmd->first.c[0], cmd->second, cmd->second); //why is cmd->second printed twice??
...
cmd2func(char c) {
  ...
  case 'S':
    // this smells fishy ;-)
    func = &csum; //rewrote func to point to csum instead of NULL
    break;
  case 'U':
...
printResults(long long result)
{
  ...
  // decimal numbers
  fprintf(stderr, "0x%llx %lld %lld\n", result, SUM_POSITIVE, SUM_NEGATIVE);
  //this will print result, SUM_POSITIVE, SUM_NEGATIVE to stderr with spaces seperating them, then put a new line at the end.
...
writeResults(long long result) {
  ...
  if (write(1, &SUM_POSITIVE, sizeof(SUM_POSITIVE)) != sizeof(SUM_POSITIVE)) { //write binary value of SUM_POSITIVE to stdout with error checking code.
    fprintf(stderr, "ERROR: write results failed\n");
  }
  if (write(1, &SUM_NEGATIVE, sizeof(SUM_NEGATIVE)) != sizeof(SUM_NEGATIVE)) { //write binary value of SUM_NEGATIVE to stdout with error checking code.
    fprintf(stderr, "ERROR: write results failed\n");
  }
  if (write(1, &CALC_DATA_BEGIN, ((void*)&CALC_DATA_END)-((void*)&CALC_DATA_BEGIN)) != ((void*)&CALC_DATA_END)-((void*)&CALC_DATA_BEGIN)) {
    fprintf(stderr, "ERROR: write results failed\n"); //write area in memory between &CALC_DATA_BEGIN and &CALC_DATA_END to stdout with error checking code.
  }
